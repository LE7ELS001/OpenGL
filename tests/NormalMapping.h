...
enum My_NormalMapping_Movement {
	NM_FORWARD,
	NM_BACKWARD,
	NM_LEFT,
	NM_RIGHT,
...
const float NM_YAW = -90.0f;
const float NM_PITCH = 0.0f;
const float NM_SPEED = 2.5f;
const float NM_SENSITIVITY = 0.1f;
const float NM_ZOOM = 45.0f;



namespace test {
	class NormalMapping : public Test
	{
	public:
		...
		NormalMapping(GLFWwindow* window);
		NormalMapping(GLFWwindow* window, float posX, float posY, float posZ, float upX, float upY, float upZ, float yaw, float pitch);
		~NormalMapping();
		...
		//键盘移动
		void ProcessKeyboard(My_NormalMapping_Movement direction, float deltaTime);
	...
	private:
		void updateNormalMappingVectors();
