...
const float LC_YAW = -90.0f;
const float LC_PITCH = 0.0f;
const float LC_SPEED = 2.5f;
const float LC_SENSITIVITY = 0.1f;
const float LC_ZOOM = 45.0f;
...
namespace test {
	...
	public:
		...
		LightCast(GLFWwindow* window);
		LightCast(GLFWwindow* window, float posX, float posY, float posZ, float upX, float upY, float upZ, float yaw, float pitch);
		~LightCast();
		...
		//键盘移动
		void ProcessKeyboard(LightCast_Movement direction, float deltaTime);
